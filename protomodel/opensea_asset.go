...
func (openseaAsset *OpenseaAsset) UnmarshalJSON(d []byte) error {
	const (
		...
		transferFeePaymentTokenJsonTag = "transfer_fee_payment_token"
	...
	data.UnmarshalString(sellOrdersJsonTag, &openseaAsset.SellOrders)
	...
	data.UnmarshalString(transferFeePaymentTokenJsonTag, &openseaAsset.TransferFeePaymentToken)
